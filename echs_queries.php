...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'add') {
        ...
        if ($cid !== '' && $q !== '') {
            $pdo->prepare("INSERT INTO echs_queries (claim_id,query_text,raised_on,qtype,priority,status,who) VALUES (?,?,?,?,?, 'open', ?)")
                ->execute([$cid, $q, $ro ?: null, $qt ?: null, $pr ?: 'medium', $who]);
        ...
        $pdo->prepare("UPDATE echs_queries SET reply_text=?, qtype=?, priority=?, assigned_to=?, due_date=?,
...
$k = $pdo->query("SELECT
        ...
        SUM(status<>'closed' AND priority='high') hi,
...
if ($search !== '') { $where[] = '(q.claim_id LIKE ? OR q.query_text LIKE ? OR c.patient_name LIKE ? OR c.esm_name LIKE ?)'; $l="%$search%"; array_push($args,$l,$l,$l,$l); }
...
$rows = $pdo->prepare("SELECT q.*, c.patient_name, c.esm_name, c.status AS claim_status, c.doctor_name
...
// ---- analytics ----
$monthly = $pdo->query("SELECT DATE_FORMAT(COALESCE(raised_on,created_at),'%Y-%m') ym, COUNT(*) n FROM echs_queries GROUP BY ym ORDER BY ym DESC LIMIT 12")->fetchAll();
$monthly = array_reverse($monthly);
$byReason = $pdo->query("SELECT COALESCE(NULLIF(qtype,''),'(untagged)') t, COUNT(*) n, SUM(status='closed') c FROM echs_queries GROUP BY t ORDER BY n DESC")->fetchAll();
$byPrio   = $pdo->query("SELECT COALESCE(NULLIF(priority,''),'medium') p, COUNT(*) n FROM echs_queries WHERE status<>'closed' GROUP BY p")->fetchAll();
$byDoctor = $pdo->query("SELECT c.doctor_name, COUNT(*) n FROM echs_queries q JOIN echs_claims c ON c.claim_id=q.claim_id COLLATE utf8mb4_unicode_ci
...
<div class="card">
    ...
    <form method="post" style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end">
        ...
        <label class="fld" style="flex:1;min-width:200px"><span class="muted small">Query / objection</span><input name="query_text" required placeholder="ECHS ne kya poocha?"></label>
...
<form class="searchbar" method="get" style="margin-bottom:6px">
    ...
    <?php if ($search||$advOpen): ?><a class="btn btn-light" href="<?= BASE_URL ?>/echs_queries.php?scheme=ECHS&st=<?= e($st) ?>">Reset</a><?php endif; ?>
...
<p class="muted small" style="margin:0 0 12px">🤖 Import ke waqt "Need More Information" claims ki query <strong>khud ban</strong> jaati hai (Auto tag), aur claim aage badhte hi <strong>khud close</strong> ho jaati hai.</p>
...
<div class="card" style="border-left:4px solid var(--warn)">
    <h2>⚠️ Portal "Need More Information" — abhi tak track nahi (<?= number_format($uncoveredN) ?>)</h2>
    <p class="muted small">Ye claims query me hain par inka koi query log nahi hua. "+ Query" se track shuru karein.</p>
    <div class="tbl-scroll"><table class="tbl">
        ...
        <?php foreach ($uncovered as $u): $ac=$u['age']>30?'style="color:#dc3545;font-weight:600"':''; ?>
            <tr><td><a class="link" href="<?= BASE_URL ?>/echs_claim.php?scheme=ECHS&id=<?= urlencode($u['claim_id']) ?>"><?= e($u['claim_id']) ?></a></td>
                <td><?= e($u['patient_name'] ?: $u['esm_name']) ?></td><td class="small"><?= e($u['status']) ?></td>
                <td class="r" <?= $ac ?>><?= (int)$u['age'] ?>d</td>
                <td class="r"><a class="btn btn-sm" href="<?= BASE_URL ?>/echs_queries.php?scheme=ECHS&for=<?= urlencode($u['claim_id']) ?>#top">+ Query</a></td></tr>
        <?php endforeach; ?>
        </tbody>
    </table></div>
</div>
...
<div class="card">
    ...
    <div class="detail-grid" style="margin-top:8px">
        <div>
            <h3 style="margin:0 0 8px">Open by priority</h3>
            <table class="tbl"><tbody>
            <?php foreach($byPrio as $p): ?><tr><td><span class="pill pill-<?= prio_pill($p['p']) ?>"><?= e(ucfirst($p['p'])) ?></span></td><td class="r"><strong><?= number_format($p['n']) ?></strong></td></tr><?php endforeach; ?>
            <?php if(!$byPrio): ?><tr><td class="muted">Koi open query nahi.</td></tr><?php endif; ?>
            </tbody></table>
        </div>
        <div>
            <h3 style="margin:0 0 8px">Doctor-wise queries (top)</h3>
            <?php if(!$byDoctor): ?><p class="muted small">Doctor data nahi (claims par doctor set karein).</p>
