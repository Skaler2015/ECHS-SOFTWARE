...
$s = $pdo->prepare("SELECT COUNT(*) n, COALESCE(SUM(claim_amt),0) claim,
    SUM(category='settled') settn,
    SUM(category IN ('rejected','cancelled')) rejn
    FROM echs_claims WHERE (YEAR(accept_date)*100+MONTH(accept_date))=?");
$s->execute([$ymn]); $sum = $s->fetch();
$recv = $pdo->prepare("SELECT COALESCE(SUM(approved_amt),0) s, COUNT(*) n FROM echs_claims WHERE category='settled' AND (YEAR(processed_on)*100+MONTH(processed_on))=?");
$recv->execute([$ymn]); $r = $recv->fetch();
$byType = $pdo->prepare("SELECT patient_type t, COUNT(*) n, COALESCE(SUM(claim_amt),0) amt FROM echs_claims WHERE (YEAR(accept_date)*100+MONTH(accept_date))=? GROUP BY t ORDER BY n DESC");
$byType->execute([$ymn]); $byType = $byType->fetchAll();
$byDoc = $pdo->prepare("SELECT doctor_name, COUNT(*) n, COALESCE(SUM(claim_amt),0) amt FROM echs_claims WHERE (YEAR(accept_date)*100+MONTH(accept_date))=? AND doctor_name IS NOT NULL AND doctor_name<>'' GROUP BY doctor_name ORDER BY n DESC LIMIT 10");
$byDoc->execute([$ymn]); $byDoc = $byDoc->fetchAll();
$hosp = APP_OWNER;
?>
<!DOCTYPE html><html lang="hi"><head><meta charset="utf-8"><title>ECHS Report <?= e($label) ?></title>
<style>
...
<div class="noprint" style="text-align:right;margin-bottom:10px">
    <form style="display:inline" method="get"><input type="hidden" name="scheme" value="ECHS"><input type="month" name="ym" value="<?= e($ym) ?>" onchange="this.form.submit()" style="padding:6px"></form>
    <button onclick="window.print()">🖨️ Print / PDF</button>
    <a href="<?= BASE_URL ?>/echs_reports.php?scheme=ECHS">← Reports</a>
</div>
<div class="head"><h1><?= e($hosp) ?></h1><div class="sub">ECHS Monthly Report — <?= e($label) ?> · Generated <?= date('d-m-Y H:i') ?></div></div>

<div class="kpis">
    <div class="kpi"><div class="v"><?= number_format($sum['n']) ?></div><div class="l">Claims accepted</div></div>
    <div class="kpi"><div class="v"><?= number_format($sum['settn']) ?></div><div class="l">Settled</div></div>
    <div class="kpi"><div class="v"><?= number_format($sum['rejn']) ?></div><div class="l">Rejected/Cancel</div></div>
...
<h2>Type-wise (OPD / IPD)</h2>
<table><thead><tr><th>Type</th><th class="r">Claims</th><th class="r">Claimed</th></tr></thead><tbody>
<?php foreach ($byType as $t): ?><tr><td><?= e(echs_ptype($t['t'])) ?></td><td class="r"><?= number_format($t['n']) ?></td><td class="r"><?= money($t['amt']) ?></td></tr><?php endforeach; ?>
